...
class Register extends Component {
  ...
  doRegister = e => {
    ...
    patient.search = `${patient.documentNumber},${patient.homePhone},${
      patient.mobilePhone
    },${patient.englishSurname},${patient.englishGivenName},${
  ...
  selectPatient = item => {
    if (JSON.stringify(item) === '{}') {
      ...
      item.patient.dateOfBirth = moment(
      ...
      <div className={'detail_warp'}>
          ...
          <form onSubmit={this.doRegister}>
            <div className={styles.form50}>
              ...
              <select
                  ...
                  value={this.state.patient.documentType}
                  onChange={(...arg) =>
                  this.changeInformation(...arg, 'documentType')
                }
              >
                {doumentTypeList.map((item, index) => (
            ...
            {basicInputs.map(item => {
              return (
                <FormInput
                    ...
                    name={item.name}
                    label={item.label}
                    value={this.state.patient[item.name]}
                    change={this.changeInformation}
                />
              );
            })}
            <div className={styles.form50}>
            ...
            <div className={styles.form50}>
              ...
              <RadioGroup
                  ...
                  onChange={(...arg) => this.changeRadio(...arg, 'sex')}
                  row
              >
                <FormControlLabel
                    value="male"
            ...
            <div className={'nephele_content_body'}>
                ...
                <Typography component={'div'}>
                  <div className={styles.form33}>
                    ...
                    <div>
                      <select
                          ...
                          value={this.state.patient.mobilePhoneAreaCode}
                          onChange={(...arg) =>
                  ...
                  <div className={styles.form33}>
                    ...
                    <div>
                      <select
                          className={'phone_select_input'}
                          value={this.state.patient.homePhoneAreaCode}
                          onChange={(...arg) =>
                          this.changeInformation(...arg, 'homePhoneAreaCode')
                        }
                      >
                        {countryCodeList.map((item, index) => (
                          <option key={index} value={item}>
                      ...
                      <InputBase
                          type={'number'}
                          className={classes.phone_form_input}
                          value={this.state.patient.homePhone}
                          onChange={(...arg) =>
                          this.changeInformation(...arg, 'homePhone')
                        }
                      />
                    </div>
                  </div>
                    ...
                    {this.state.showAddress ? (
                      <div className={styles.form_content}>
                        ...
                        <div className={styles.form15}>
                          ...
                          <select
                              className={'select_input'}
                              value={this.state.patient.region}
                              onChange={(...arg) =>
                              this.changeInformation(...arg, 'region')
                            }
                          >
                        ...
                        <div className={styles.form38}>
                          ...
                          <select
                              className={'select_input'}
                              value={this.state.patient.district}
                              onChange={(...arg) =>
                              this.changeInformation(...arg, 'district')
                            }
                          >
              ...
              {this.state.tabValue === 1 && (
                <Typography component={'div'}>
                  {this.state.contactPersonList.length > 0
                    ? this.state.contactPersonList.map((item, index) => (
                        <Contact
                            key={index}
                            index={index}
                            contact={item}
                  ...
                  >
                    <Add className={classes.add_icon} color={'primary'} />
                    <span>Add more contact person</span>
                  </div>
                </Typography>
              )}
            </div>
        ...
        <Dialog open={this.state.loading && this.props.patientLoading}>
          {this.props.patientErrorMessage === '' ? (
            ...
            <DialogContent>{this.props.patientErrorMessage}</DialogContent>
          )}
          {this.props.patientErrorMessage !== '' ? (
            <DialogActions>
...
export default withRouter(
  connect(mapStateToProps)(withStyles(style)(Register))
);
